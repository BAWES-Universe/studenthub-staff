...
let AuthHttpService = class AuthHttpService {
    constructor(_http, _auth, _alertCtrl, eventService, translateService) {
        this._http = _http;
        this._auth = _auth;
     ...
     * @param filename
     */
    generateCards(endpointUrl, params, filename) {
    ...
    getRaw(endpointUrl) {
        ...
        return this._http.get(url, { headers: headers, observe: 'response' })
            .pipe(retryWhen(genericRetryStrategy()), catchError((err) => this._handleError(err)), take(1), map((res) => { return res; }));
    }
    /**
     * Requests via GET verb
     * @param {string} endpointUrl
    ...
    pdfget(endpointUrl, filename) {
        ...
        }).pipe(retryWhen(genericRetryStrategy()), map((response) => {
     ...
     * @param withHeader
     */
    post(endpointUrl, params, withHeader = false) {
        const url = environment.apiEndpoint + endpointUrl;
        const headers = this._buildAuthHeaders();
        ...
        return this._http.post(url, JSON.stringify(params), { headers: headers, observe: 'response' })
            .pipe(retryWhen(genericRetryStrategy()), catchError((err) => this._handleError(err)), take(1), map((res) => {
     ...
     * @param formData
     */
    uploadFile(endpointUrl, formData) {
        const url = environment.apiEndpoint + endpointUrl;
        ...
        return this._http.post(url, formData, { headers: headers })
            .pipe(retryWhen(genericRetryStrategy()), catchError((err) => this._handleError(err)), map((res) => { return res; }));
    }
    /**
